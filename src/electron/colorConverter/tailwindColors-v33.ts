...
const colours: { [key: string]: string | { [key: number]: string } } = {
  black: '#000',
  white: '#fff',
  slate: {
    50: '#f8fafc',
...
const tailwindColours: { [key: string]: string } = {}

for (const coloursKey in colours) {
  if (typeof colours[coloursKey] === 'string') {
    tailwindColours[coloursKey] = colours[coloursKey] as string
  } else {
    for (const nestedKey in colours[coloursKey] as { [key: number]: string }) {
